...
function createKeyboardRows() {
  ...
  for (let i = 0; i < numberOfButtonsInARow.length; i += 1) {
    const row = arr.slice(start, end);
...
const ctrlBtn = document.querySelectorAll('.control');
const altBtn = document.querySelectorAll('.alt');
...
simpleBtns.forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    setTimeout(() => {
      buttons.forEach((el) => el.classList.remove('end'));
    }, 200);
  });
});
...
function shiftHandlerDown() {
  ...
  if (lang === 'en') {
    simpleBtns.forEach((btn, i) => {
      simpleBtns[i].innerHTML = simpleBtnsEn[i];
      btn.classList.add('upper-case');
    });
  } else {
    simpleBtns.forEach((btn, i) => {
      simpleBtns[i].innerHTML = simpleBtnsRu[i];
      btn.classList.add('upper-case');
    });
  }
}
function shiftHandlerUp() {
  ...
  if (lang === 'en') {
    simpleBtns.forEach((btn, i) => {
      simpleBtns[i].innerHTML = simpleBtnsEnUsual[i];
      btn.classList.remove('upper-case');
    });
  } else {
    simpleBtns.forEach((btn, i) => {
      simpleBtns[i].innerHTML = simpleBtnsRuUsual[i];
      btn.classList.remove('upper-case');
    });
  }
}
...
document.addEventListener('keydown', (event) => {
  ...
  for (let i = 0; i < simpleBtns.length; i += 1) {
